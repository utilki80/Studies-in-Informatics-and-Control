...
figure(1); 
subplot(211); 
plot(y.time,y.data(:,1),'k',y.time,y.data(:,7),'r:','linewidth',2); grid,  
xlabel('time(s)');ylabel('Link 1 Trajectory'); 
legend('Reference Trajectory','NNBATSMCWGA Solution'); 
subplot(212); 
plot(y.time,y.data(:,4),'k',y.time,y.data(:,9),'r:','linewidth',2); grid, 
xlabel('time(s)');
ylabel('Link 2 Trajectory'); 
legend('Reference Trajectory','NNBATSMCWGA Solution'); 
